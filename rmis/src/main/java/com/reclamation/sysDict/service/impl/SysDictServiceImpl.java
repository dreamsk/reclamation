package com.reclamation.sysDict.service.impl;

import com.github.pagehelper.PageHelper;
import com.github.pagehelper.PageInfo;
import com.reclamation.base.BaseService;
import com.reclamation.mapper.SysDictMapper;
...
public class SysDictServiceImpl extends BaseService implements SysDictService {
    ...
    public DataTables getPageList(DataTables dataTables) {
        ...
        if(!StringUtils.isEmpty(property)){
            ...
            //使用工具类进行转化
            String column =  propertyToColumn(SysDictMapper.class,property);
            if(!StringUtils.isEmpty(column)){
                //参数排序
                PageHelper.orderBy(column + " "+dataTables.getOrder());
            }
        }
        List<SysDict> menus = sysDictMapper.getPageList(dataTables.getSearch(),dataTables.getSubSQL());
        //pageinfo完成分页处理
        PageInfo<SysDict> pageInfo = new PageInfo<SysDict>(menus);
        List<SysDict> data = new ArrayList<SysDict>();
    ...
    @Override
    public Map<String, String> add(SysDict sysDict) {
        Map<String, String> map = new HashMap<String, String>();
        int res = sysDictMapper.insertSelective(sysDict);
        if(res > 0){
            map.put("status", "200");
        }else {
            map.put("status", "999");
    ...
    @Override
    public Map<String, String> update(SysDict sysDict) {
        Map<String, String> map = new HashMap<String, String>();
        int res = sysDictMapper.updateByPrimaryKeySelective(sysDict);
        if(res > 0){
            map.put("status", "200");
        }else {
            map.put("status", "999");
    ...
    public Map<String, String> del(List<Integer> idlist) {
        Map<String, String> map = new HashMap<String, String>();
        int res = sysDictMapper.deleteByPrimaryKeys(idlist);
        if(res > 0){
            map.put("status", "200");
        }else {
            map.put("status", "999");
    ...
    @Override
    public List<SysDict> selectAll(String dict_tabname) {
