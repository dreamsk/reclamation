...
$(function($) {
	...
	if(entityID) {
		$.post(contextPath + "/scrapCollector/getScrapCollectorById", {
			"id": entityID
		}, function(result) {
	...
	$("#form-scrap-collector-add").validate({
		...
		submitHandler: function(form) {
			$.ajax({
				type: "post",
				url: contextPath + "/scrapCollector/" + (entityID ? "update" : "add"),
				data: $(form).serialize(),
				dataType: "json",
...
function createHousingEstateTree(){
	var setting = {
			async: {
				...
				type: "post",
				url: contextPath + "/housingEstate/getHousingEstateByCityCode"
			},
			view: {
				selectedMulti: false
			},
...
function zTreeOnClickHousingEstate(event, treeId, treeNode) {
	$('#housingEstate').val(treeNode.name);
